...
namespace Coral {
	...
	bool HostInstance::Initialize(HostSettings InSettings)
	{
		...
		m_CoralManagedAssemblyPath = std::filesystem::path(m_Settings.CoralDirectory) / m_Settings.CoralFilename;

		if (!std::filesystem::exists(m_CoralManagedAssemblyPath))
		{
			MessageCallback("Failed to find " + m_Settings.CoralFilename, MessageLevel::Error);
			return false;
		}
