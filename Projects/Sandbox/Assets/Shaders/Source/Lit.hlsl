...
//float3 phongLighting(float3 n, // The normalized surface normal vector.
//                     float3 l, // The normalized direction from the surface point to the light source (i.e. constant for directional lights)
//                     float3 v, // The normalized direction from the surface point to the eye.
//                     float3 Kd, // The diffuse reflection coefficient of the surface material.
//                     float3 Ks, // The specular reflection coefficient of the surface material.
//                     float Ns, // The specular exponent of the surface material.
//                     float3 E_directional // The irradiance of the directional light source.
//                    )
//{
// 
//    const float3 r = reflect(-l, n); // Calculate the reflection vector using HLSL intrinsics.
//    const float n_dot_l = saturate(dot(n, l)); // Note the saturate!
//    const float v_dot_r = saturate(dot(v, r)); // Note the saturate again!
//    
//    const float3 diffuse = Kd * Inverse_PI; // Evaluate the diffuse part of the Phong BRDF.
//    const float3 specular = Ks * Inverse_PI * pow(v_dot_r, Ns); // Evaluate the specular part of the Phong BRDF.
//    const float3 brdf = diffuse + specular; // Evaluate the complete Phong BRDF.
//    const float3 radiance = brdf * E_directional * n_dot_l; // Combine the BRDF and the irradiance.
//    
//    return radiance;
//}
