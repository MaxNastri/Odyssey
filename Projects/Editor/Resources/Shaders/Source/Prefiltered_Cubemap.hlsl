...
cbuffer PrefilteredData : register(b0)
{
    float4x4 MVP;
    float Roughness;
    uint SampleCount;
...
float3 prefilterEnvMap(float3 R, float roughness)
{
    ...
    int2 envMapDims;
    skyboxTex3D.GetDimensions(envMapDims.x, envMapDims.y);
    ...
    for (uint i = 0u; i < SampleCount; i++)
    {
        ...
        if (dotNL > 0.0)
        {
            ...
            color += skyboxTex3D.SampleLevel(skyboxSampler, L, mipLevel).rgb * dotNL;
            totalWeight += dotNL;
...
float4 main(FragInput input) : SV_Target
{
    ...
    float3 finalColor = prefilterEnvMap(N, Roughness);
