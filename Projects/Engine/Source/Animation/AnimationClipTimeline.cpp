...
namespace Odyssey
{
	...
	const std::unordered_map<std::string, glm::mat4>& AnimationClipTimeline::BlendKeys(double dt)
	{
		m_CurrentTime += dt;
		...
		for (auto& [boneName, boneKeyframe] : boneKeyframes)
		{
			...
			// Blend the keys
			m_BoneKeys[boneName] = boneKeyframe.BlendKeys(m_PrevFrame, m_NextFrame, blendFactor);
		}
