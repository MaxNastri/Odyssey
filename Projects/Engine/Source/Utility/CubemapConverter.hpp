	...
	: m_cubemapResolution(cubemapResolution), m_filterLinear(filterLinear)
{
	...
	m_imageData = stbi_load(pathHdri.c_str(), &m_width, &m_height, &m_channels, 4);
	m_channels = 4;
	...
	: m_cubemapResolution(cubemapResolution), m_filterLinear(filterLinear)
{
	...
	m_imageData = stbi_loadf(pathHdri.c_str(), &m_width, &m_height, &m_channels, 4);
	m_channels = 4;
			...
			for (int col = 0; col < m_cubemapResolution; col++)
			{
				...
				else // perform bilinear interpolation
				{
					...
					for (int j = 0; j < m_channels; j++)
					{
						...
						{
							float interpolatedValue = m_imageData[low_idx_column * m_channels + m_width * low_idx_row * m_channels + j] * f1 +
